...
恒等マッピングはsupervisor・書き込み可能・実行可能のまま保持します。
...
ガードページ、カーネル本体の高位配置、コード・データ別の保護、プロセス別アドレス空間は今後の段階です。
...
`make test` は生成物の検証に加え、次の15ケースをQEMUで逐次実行します。
...
次の段階では、ガードページやカーネルのコード・データ別のページ保護を小さく追加していきます。
