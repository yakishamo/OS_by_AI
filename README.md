...
- カーネルは専用スタックを使います。GDT・IDT・TSSとページテーブルをカーネル専用のものに切り替えます。スタックのガードページやコード・データ別のページ保護は未実装です。
...
現段階のマッピングはすべてsupervisor・書き込み可能・実行可能です。
動的なmap/unmap、NX・読み取り専用の保護、ガードページ、高位アドレス配置、プロセス別アドレス空間は今後の段階です。
...
`make test` は生成物の検証に加え、次の12ケースをQEMUで逐次実行します。

- 本物の `kernel.elf` を起動し、`kernel_halt`でのRIP、`HLT=1`、`IF=0`、RIP直前のHLT命令、およびページテーブル切り替え後のRAMテスト成功を含むシリアルログ6行をCRLFも含めて確認。
...
次の段階では、ページのmap/unmapとアクセス権限の管理を小さく追加していきます。
