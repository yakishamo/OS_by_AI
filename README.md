...
Boot Servicesを終了します。その後、起動情報をカーネルに渡し、専用スタックへ切り替えてから内容を確認し、カーネル内のデバッグ用コンソールを起動します。
...
カーネルは起動情報とスタック位置を確認し、自身のシリアルドライバーでログを出してコマンド入力を待ちます。
...
ログの後、`K> ` プロンプトでコマンドを入力できます。`halt` を実行するとHLTで停止します。
`BOOT:` はBoot Services終了前のローダーによる出力、`KERNEL:` は終了後のカーネルによる出力です。

## デバッグ用コンソール

`kernel/console.c` はカーネル内で動く開発用コンソールです。`make run`（または `gmake run`）で利用できます。

| コマンド | 動作 |
|---|---|
| `help` | コマンド一覧 |
| `mem` | 4 KiB物理ページの総数・使用数・空き数 |
| `clear` | ANSIエスケープシーケンスで画面をクリア |
| `halt` | 出力を完了してCPUを停止。再開にはQEMUの再起動が必要 |

入力はASCIIで最大127文字、固定長バッファを使います。EnterはCR・LF・CRLFに対応し、CRLFを二重実行しません。
BackspaceとDeleteで末尾を削除し、Ctrl-Cで入力を取り消します。タブは1つの空白に変換し、前後の空白は無視します。
履歴やカーソル移動は未実装で、矢印キーのCSI/SS3シーケンスは読み捨てます。
長すぎる行とUART受信エラーが起きた行は、Enterまで破棄してからエラーを表示します。
未知のコマンドや引数付きのコマンドは実行しません。ファイル操作や外部プログラムの起動機能はありません。

割り込みは無効のまま、入力がない間はPAUSEでポーリングします。アイドル時もホストCPUを使用します。
...
成功後、`kernel_boot_info` にポインターを保持し、初期化と自己テストを終えてコンソールを起動します。
