...
初期化とメモリ自己テストは割り込み無効で実行し、その後PIC/PITを設定してタイマー割り込みを許可します。

例外入口は、CPUがエラーコードを積まないベクターには0を補い、番号とエラーコードを共通形式にします。
Cのハンドラーは番号、エラーコード、RIP、CS、RFLAGS、例外前のRSPをシリアルへ出力し、
ページフォルトではCR2も出力して停止します。
CPU例外は引き続き致命的として扱い復帰しません。外部IRQ（ベクター32〜47）は別の入口を使い、汎用レジスターを保存・復元してIRETQで復帰します。

ダブルフォルト用スタックはカーネルの `.stacks` 領域にあり、通常のスタックが壊れた場合にも例外を報告できるようにしています。
...
IRQ内ではログ出力やメモリ確保を行いません。IRQ7/15のspurious割り込みを判別し、不要なEOIを避けます。
その他の予期しないPIC IRQは記録してマスクし、EOIを送ります。

起動時には実際の割り込みを1回受け、全汎用レジスターとDF/CFの保存を自己テストします。
その後、割り込み有効のままコンソールへ進みます。メモリ管理APIは引き続き単一CPU・割り込み無効の契約です。
今後コンソールなどからメモリを変更する場合は、呼び出し側で割り込み状態を管理する必要があります。
`timer_ticks()` はこのCPUが受信したIRQ0の回数です。長時間の割り込み禁止やQEMU停止中の経過時間は保証せず、壁時計ではありません。
スケジューラーやタスク切り替えはまだ実装していません。
